...
def _extract_vaild_additional_channels_pyfunc(additional_channel_tensor, pad_num_tensor):
    assert(int(pad_num_tensor) >= 0) 
    out_img_list = []
    vaild_channel = None
    if pad_num_tensor > 0:
        pad_num = -int(pad_num_tensor)
        vaild_channel = additional_channel_tensor[:, :, :pad_num]
    else:
        vaild_channel = additional_channel_tensor[:, :, :]
    # Append to output list
    out_img_list.append(vaild_channel)
    return out_img_list
...
# Writen by Roy;
# Using different way to implement 'Additional channels merging' function
# Using PNG decode
class myTfExampleDecoder(data_decoder.DataDecoder):
  """Tensorflow Example proto decoder."""
  ...
  def __init__(self,
               ...
               dct_method='',
               num_keypoints=0,
               num_vaild_additional_channels=0,  # added by Roy
               extract_vaild_additional_channels_fn=_extract_vaild_additional_channels_pyfunc,
               load_multiclass_scores=False,
               load_context_features=False,
               expand_hierarchy_labels=False):
    """Constructor sets keys_to_features and items_to_handlers.
    ...
    # other decoders to handle label maps similarly.
    del use_display_name
    assert(num_vaild_additional_channels <= 4)   # 目前最多支持4个额外通道的输入
    self._num_vaild_additional_channels = num_vaild_additional_channels
    self.extract_vaild_additional_channels_fn = extract_vaild_additional_channels_fn
    self.keys_to_features = {
        'image/encoded':
            tf.FixedLenFeature((), tf.string, default_value=''),
        'image/format':
    ...
    # We are checking `dct_method` instead of passing it directly in order to
    # ensure TF version 1.6 compatibility.
    if self._num_vaild_additional_channels > 0:
      padding_num_handle = slim_example_decoder.Tensor('image/padding_num')
      if dct_method:
        image = slim_example_decoder.Image(
            image_key='image/encoded',
            format_key='image/format',
            ...
            dct_method=dct_method)
        additional_channel_image = slim_example_decoder.Image(
            image_key='image/attach_channels',
            format_key='image/format',
            channels=4,
            dct_method=dct_method)
      else:
    ...
    if(additional_channel_image is not None):
      assert(padding_num_handle is not None)
    ...
    # Insert additional channels into map (added by Roy)
    if additional_channel_image and padding_num_handle:
      self.keys_to_features['image/attach_channels'] = tf.FixedLenFeature([], tf.string, default_value='')
      self.keys_to_features['image/padding_num'] = tf.FixedLenFeature([], tf.int64, default_value=-1)
      self.items_to_handlers[fields.InputDataFields.image_additional_channels] = additional_channel_image
      self.items_to_handlers[fields.InputDataFields.additional_channels_padding_num] = padding_num_handle
  ...
  def decode(self, tf_example_string_tensor):
    ...
    tensor_dict[is_crowd] = tf.cast(tensor_dict[is_crowd], dtype=tf.bool)
    tensor_dict[fields.InputDataFields.image].set_shape([None, None, 3])
    tensor_dict[fields.InputDataFields.original_image] = tensor_dict[fields.InputDataFields.image]
    tensor_dict[fields.InputDataFields.original_image_spatial_shape] = tf.shape(
        tensor_dict[fields.InputDataFields.image])[:2]
    
    # TODO:Concatenate the image and the additional vaild channels
    if (fields.InputDataFields.image_additional_channels in tensor_dict) \
      and (fields.InputDataFields.additional_channels_padding_num in tensor_dict):
      tensor_dict[fields.InputDataFields.image_additional_channels].set_shape([None, None, 4])
      channels_tensor = tensor_dict[fields.InputDataFields.image_additional_channels]
      pad_num_tensor = tensor_dict[fields.InputDataFields.additional_channels_padding_num]
      vaild_channels_tensor = tf.py_func(
          self.extract_vaild_additional_channels_fn, 
          [channels_tensor, pad_num_tensor], 
          [tf.uint8]
      )
      vaild_channels_tensor[0].set_shape([None, None, self._num_vaild_additional_channels])
      tensor_dict[fields.InputDataFields.image_additional_channels] = vaild_channels_tensor[0]  # concat操作留在inputs里面操作
      # For debug
      # print("vaild_channels_tensor shape:{}".format(vaild_channels_tensor))
      # input("Continue...")
